...
The collapsible **Doom H3IMG uploader** loads a packaged `.h3img` Doom image through the same Web Serial connection used by the UART terminal. It does not write the SD card. The resident monitor must be active at its `>` prompt; if Doom is running, stop Doom first and wait for the prompt before starting the upload.
