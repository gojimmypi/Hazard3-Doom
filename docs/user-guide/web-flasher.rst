...
On Windows, direct WebUSB access to the ULX3S FT231X requires its USB interface
to be bound to the **WinUSB** driver. The normal FTDI VCP/D2XX driver does not
allow the browser to open this interface through WebUSB.
...
One way to change the driver is with Zadig:
...
.. warning::
   ...
   Rebinding the FT231X to WinUSB makes software that expects the normal FTDI
   VCP/D2XX driver unavailable on that interface until the FTDI driver is
   restored. The Hazard3-Doom UART console can still use a separate external
   USB-to-UART adapter while ``US1`` is bound to WinUSB.
...
.. figure:: ../images/Windows-set-default-USB-from-WinUSB.png
   ...
   Device Manager can be used to restore the normal FTDI driver after WebUSB
