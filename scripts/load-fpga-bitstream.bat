...
if exist "%ROOT_DIR%\build\hazard3-boot-monitor.bit" (
    ...
    if exist "%ROOT_DIR%\bin\fpga_ulx3s_85f_hdmi_doom.bit" (
        set "SOURCE_BITSTREAM=%ROOT_DIR%\bin\fpga_ulx3s_85f_hdmi_doom.bit"
        ...
        >&2 echo   "%ROOT_DIR%\bin\fpga_ulx3s_85f_hdmi_doom.bit"
        >&2 echo.
        goto ERROR_EXIT
    )
)
